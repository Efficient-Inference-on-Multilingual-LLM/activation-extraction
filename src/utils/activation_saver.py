...
class ActivationSaver:
	...
	def hook_fn(self, module, input, output, layer_id):
		...
		if any(list(map(lambda x: x in self.model_name.lower(), ['gemma', 'pythia']))) and layer_id not in ['embed_tokens', 'norm']:
	...
	def pre_hook_fn(self, module, input, layer_id):
		...
		save_path = os.path.join(self.base_save_dir, self.task_id, self.data_split, self.model_name.split('/')[-1], self.prompt_id, self.current_lang, self.current_id, "last_token")
